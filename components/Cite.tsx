...
export function Cite({ source }: { source: Source }) {
  return (
    <span className="group relative inline-flex items-baseline">
      ...
      <span
        role="note"
        className="pointer-events-none absolute bottom-full left-0 z-10 mb-2 w-72 rounded-sm border border-line bg-card p-3 text-xs leading-relaxed text-muted opacity-0 shadow-lg transition-opacity group-hover:opacity-100 group-focus-within:opacity-100"
      >
        {source.label}
