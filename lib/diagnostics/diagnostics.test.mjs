...
test("coverage no longer separates the two deep dives, depth does", () => {
  // this replaces an assertion that no two registers covered the same pillars.
  // that was true while one name had only a research note behind it. reading
  // the Anant Raj annual report closed the gap, so the claim is now about what
  // is underneath identical coverage rather than about the coverage itself
  const set = (reg) => [...new Set(reg.rows.map((r) => r.category))].sort().join(",");
  assert.equal(set(areg), set(rreg),
    "the two deep dives have stopped covering the same pillars, so the claim below needs rewriting");
  ...
  assert.ok(set(nw) !== set(rreg), "Netweb now covers the same pillars as the filed register");
});
