...
const pillars = JSON.parse(readFileSync(new URL("../../data/pillars.json", import.meta.url)));
test("every filled coverage cell names a subject that exists", () => {
  const ids = new Set(pillars.subjects.map((s) => s.id));
  for (const p of pillars.pillars) {
    for (const c of p.covered) assert.ok(ids.has(c), `${p.id} claims unknown subject ${c}`);
    assert.equal(new Set(p.covered).size, p.covered.length, `${p.id} counts a subject twice`);
  }
});

test("each pillar state describes the coverage it claims", () => {
  // the states are mechanical rather than editorial, so a mislabelled one fails
  const half = pillars.subjects.length / 2;
  for (const p of pillars.pillars) {
    const n = p.covered.length;
    if (p.state === "BUILT") assert.ok(n > half, `${p.id} is BUILT on ${n}`);
    else if (p.state === "TERMINAL") assert.equal(n, 0, `${p.id} is TERMINAL on ${n}`);
    else assert.ok(n > 0 && n <= half, `${p.id} is ${p.state} on ${n}`);
  }
});

test("the coverage count is what the cells actually add up to", () => {
  // the page publishes this as its own completeness, so it is summed rather
  // than stated
  // the count is derived at render, so pinning its value here would encode a
  // state rather than a rule and would fail every time work lands. what has to
  // hold is that the published number is the cells and nothing else
  const filled = pillars.pillars.reduce((t, p) => t + p.covered.length, 0);
  const total = pillars.subjects.length * pillars.pillars.length;
  const distinct = pillars.pillars.reduce((t, p) => t + new Set(p.covered).size, 0);
  assert.equal(filled, distinct, "a subject is counted twice in some pillar");
  assert.ok(filled > 0 && filled < total, `coverage is ${filled} of ${total}`);
  assert.equal(total, pillars.subjects.length * 6, "the brief names six pillars");
});

test("no filer restates more points than were harvested from it", () => {
  for (const c of [co, ...others]) {
    const r = c.restatements;
    assert.ok(r.rows.length <= r.annualPoints, `${c.ticker} restates ${r.rows.length} of ${r.annualPoints}`);
    const seen = new Set(r.rows.map((x) => `${x.concept}@${x.period}`));
    assert.equal(seen.size, r.rows.length, `${c.ticker} counts a restated point twice`);
  }
});

test("the restatement rate is read against an unequal denominator", () => {
  // the finding needs both numbers. if every filer were asked the same number of
  // points the page would not need to draw the denominator and its prose would
  // be wrong
  const points = [co, ...others].map((c) => c.restatements.annualPoints);
  assert.ok(new Set(points).size > 1, "every filer now has the same denominator");
  const rates = [co, ...others].map((c) => ({
    t: c.ticker,
    rate: c.restatements.rows.length / c.restatements.annualPoints,
    points: c.restatements.annualPoints,
  }));
  const worstRate = [...rates].sort((a, b) => b.rate - a.rate)[0];
  const biggest = [...rates].sort((a, b) => b.points - a.points)[0];
  assert.notEqual(worstRate.t, biggest.t,
    "the filer restating most is now also the one asked most, so the correction the page makes is moot");
});

test("three of the five filers restated nothing", () => {
  const clean = [co, ...others].filter((c) => c.restatements.rows.length === 0);
  assert.equal(clean.length, 3, `${clean.length} filers now restate nothing`);
});

// ---- the README, checked against the data it quotes ----
//
// The README carried three mutually inconsistent counts at once, because
// nothing checked it. It is the most read file here and the least guarded, so
// every headline figure in it is asserted against the same file the site
// derives from. A figure that moves fails here rather than misinforming a
// reader who never opens the repository.
const readme = readFileSync(new URL("../../README.md", import.meta.url), "utf8");
const says = (s) => readme.includes(s);

test("the README quotes the capacity ladder as the filing prints it", () => {
  for (const r of pros.capacity.rungs) {
    assert.ok(says(String(r.mw)), `the README no longer quotes ${r.name} at ${r.mw}`);
  }
});

test("the README quotes the invariant and test counts it actually has", () => {
  const invariantCount = inv.rows.length;
  assert.ok(says(`${invariantCount} invariants`) || says(`${invariantCount} of them`),
    `the README does not state ${invariantCount} invariants`);
  // The test count is asserted as a floor rather than exactly. Pinning it would
  // mean every new test forces a README edit, which is a state wearing the
  // clothes of a rule. What must not happen is the README claiming more than
  // the suite actually contains.
  const testCount = (readFileSync(new URL("./diagnostics.test.mjs", import.meta.url), "utf8")
    .match(/^test\(/gm) ?? []).length;
  const claimed = Number((readme.match(/([0-9]+) tests/) || [])[1]);
  assert.ok(Number.isFinite(claimed), "the README no longer states a test count");
  assert.ok(claimed <= testCount, `the README claims ${claimed} tests against ${testCount} that exist`);
});

test("the README quotes the coverage count the matrix computes", () => {
  const filled = pillars.pillars.reduce((t, p) => t + p.covered.length, 0);
  const total = pillars.subjects.length * pillars.pillars.length;
  assert.ok(says(`${filled} of ${total}`), `coverage is ${filled} of ${total} and the README differs`);
});

test("the README quotes the Techno Electric figures from the data file", () => {
  for (const n of [
    te.commitments.capitalCommitmentMn,
    te.payables.msmePrincipalMn,
    te.supplierFinance.reclassifiedMn,
    te.governance.emphasisOfMatter.amountMn,
    te.target.mw,
    te.segmentation.printedPagesSearched,
  ]) {
    assert.ok(says(String(n)), `the README no longer quotes ${n}`);
  }
});

test("the README quotes the Anant Raj delivery ladder", () => {
  for (const r of ar.rungs) {
    if (r.kind === "AMBITION") assert.ok(says(String(r.mw)), `announced ${r.mw} is not in the README`);
  }
});
...
test("the supplier finance transfer is most of the standalone current borrowings", () => {
  // a liability that changed line without cash changing hands. the share is the
  // finding, so it is asserted rather than the amount alone
  const sf = te.supplierFinance;
  const share = sf.reclassifiedMn / sf.standaloneCurrentBorrowingsMn;
  assert.ok(share > 0.9, `the transfer is ${share} of current borrowings`);
  assert.ok(sf.reclassifiedMn <= sf.standaloneCurrentBorrowingsMn,
    "more was transferred in than the line it landed on reports");
  assert.equal(sf.carryingAmountPriorMn, 0, "the arrangement is no longer new in this year");
});

test("the arrangement buys more time than the payables it replaced", () => {
  const sf = te.supplierFinance;
  assert.ok(sf.arrangementDays.low >= sf.comparableDays.high);
  assert.ok(sf.arrangementDays.low < sf.arrangementDays.high);
  assert.ok(sf.comparableDays.low < sf.comparableDays.high);
});

test("the supplier finance is larger than the capital the company contracted", () => {
  assert.ok(te.supplierFinance.carryingAmountMn > te.commitments.capitalCommitmentMn);
  // and the counterweight the page also states: against the payables it came out
  // of, the amount is small. both readings have to survive
  const share = te.supplierFinance.reclassifiedMn / te.supplierFinance.tradePayablesMn;
  assert.ok(share < 0.05, `the transfer is now ${share} of trade payables, so it is no longer small`);
});

test("the whole micro and small enterprise balance sits past its due date", () => {
  // the ageing columns run from the due date, so anything outside the first is late
  const pay = te.payables;
  const notDue = pay.ageing.find((r) => r.bucket === "NOT_DUE");
  assert.equal(notDue.msmeMn, 0, "some micro and small dues are no longer overdue");
  assert.equal(notDue.msmePriorMn, 0);
  assert.ok(pay.msmePrincipalMn > pay.msmePrincipalPriorMn, "the balance no longer rose");
});

test("all five statutory interest clauses report nothing", () => {
  const nil = te.payables.msmedClauses.filter((c) => c.interestMn === 0 && c.interestPriorMn === 0);
  assert.equal(nil.length, 5, "a clause now reports an amount");
  assert.deepEqual(te.payables.msmedClauses.map((c) => c.clause), ["a", "b", "c", "d", "e"]);
});

test("the ageing buckets sum to the totals the filing prints", () => {
  // twenty transcribed cells. a mistyped bucket would leave every derived share
  // slightly wrong and nothing else would catch it
  const pay = te.payables;
  const cur = pay.ageing.reduce((t, r) => t + r.msmeMn + r.othersMn, 0);
  const pri = pay.ageing.reduce((t, r) => t + r.msmePriorMn + r.othersPriorMn, 0);
  assert.ok(Math.abs(cur - pay.totalMn) < 0.01, `buckets sum to ${cur} against ${pay.totalMn}`);
  assert.ok(Math.abs(pri - pay.totalPriorMn) < 0.01, `prior buckets sum to ${pri}`);
});
